	...
	*/
	...
	export enum WrapMode {		
		...
		*/
		...
		/** !#en Returns the position in pixels of a given mapTile coordinate.
		!#zh 获取指定 mapTile 的像素坐标。
		...
		*/
		...
		/** !#en Removes a mapTile at given mapTile coordinate.
		!#zh 删除指定坐标上的 mapTile。
		@param pos position or x
		...
		*/
		...
		/** !#en
		Sets the mapTile gid (gid = mapTile global id) at a given mapTile coordinate.<br />
		The Tile GID can be obtained by using the method "tileGIDAt" or by using the TMX editor . Tileset Mgr +1.<br />
		If a mapTile is already placed at that position, then it will be removed.
		!#zh
		设置给定坐标的 mapTile 的 gid (gid = mapTile 全局 id)，
		mapTile 的 GID 可以使用方法 “tileGIDAt” 来获得。<br />
		如果一个 mapTile 已经放在那个位置，那么它将被删除。
		...
		*/
		...
		/** !#en
		Returns the mapTile gid at a given mapTile coordinate. <br />
		if it returns 0, it means that the mapTile is empty. <br />
		This method requires the the mapTile map has not been previously released (eg. don't call layer.releaseMap())<br />
		!#zh
		通过给定的 mapTile 坐标、flags（可选）返回 mapTile 的 GID. <br />
		如果它返回 0，则表示该 mapTile 为空。<br />
		该方法要求 mapTile 地图之前没有被释放过(如：没有调用过layer.releaseMap()).
		@param pos or x
		...
		*/
		...
		/** !#en
		Returns the mapTile (_ccsg.Sprite) at a given a mapTile coordinate. <br/>
		...
		!#zh
		通过指定的 mapTile 坐标获取对应的 mapTile(Sprite)。 返回的 mapTile(Sprite) 应是已经添加到 TMXLayer，请不要重复添加。<br/>
		这个 mapTile(Sprite) 如同其他的 Sprite 一样，可以旋转、缩放、翻转、透明化、设置颜色等。<br/>
		...
		*/
		...
		/** !#en
		Dealloc the map that contains the mapTile position from memory. <br />
		...
		*/
		...
		/** !#en Size of the map's mapTile (could be different from the mapTile's size).
		!#zh 获取 mapTile 的大小( mapTile 的大小可能会有所不同)。
		...
		*/
		...
		/** !#en Gets the mapTile size.
		!#zh 获取地图背景中 mapTile 元素的大小。
		...
		*/
		getTileSize() : Size;		
		/** !#en Set the mapTile size.
		!#zh 设置地图背景中 mapTile 元素的大小。
		...
		*/
		...
		/** !#en Return properties dictionary for mapTile GID.
		!#zh 通过 GID ，获取指定的属性。
